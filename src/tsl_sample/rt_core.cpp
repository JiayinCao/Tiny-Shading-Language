 ...
 * doesn't need to come with a sophisticated ray tracer.
 *
...
Vec radiance(const Ray& r, int depth, unsigned short* Xi) {
    ...
    if (++depth > 5) if (random_numbert() < p && depth < 10) f = f * (1 / p); else return obj.e; //R.R. 
    if (obj.refl == DIFF) {                  // Ideal DIFFUSE reflection 
        double r1 = 2 * 3.1415926f * random_numbert(), r2 = random_numbert(), r2s = sqrt(r2);
    ...
    return obj.e + f.mult(depth > 2 ? (random_numbert() < P ?   // Russian roulette 
...
int rt_main() {
            ...
            for (int sy = 0, i = (h - y - 1) * w + x; sy < 2; sy++)     // 2x2 subpixel rows 
                for (int sx = 0; sx < 2; sx++, r = Vec()) {        // 2x2 subpixel cols 
                    for (int s = 0; s < samps; s++) {
                        double r1 = 2 * random_numbert(), dx = r1 < 1 ? sqrt(r1) - 1 : 1 - sqrt(2 - r1);
                        double r2 = 2 * random_numbert(), dy = r2 < 1 ? sqrt(r2) - 1 : 1 - sqrt(2 - r2);
    ...
    char* ret = new char[w * h * 3];
    for (int i = 0; i < w * h; i++) {
        ret[i * 3] = toInt(c[i].x);
        ret[i * 3 + 1] = toInt(c[i].y);
        ret[i * 3 + 2] = toInt(c[i].z);
    }
    stbi_write_jpg("tsl_sample.jpg", w, h, 3, ret, 100);
    return 0;
}