...
void GlobalModule::declare_global_module(LLVM_Compile_Context& context){
    ...
    // texture 2d sampling
    Function* texture2d_sample_function = Function::Create(FunctionType::get(get_void_ty(context), { get_int_32_ptr_ty(context) , get_float_ty(context), get_float_ty(context), float3_struct_llvm_type->getPointerTo(), get_float_ptr_ty(context) }, false), Function::ExternalLinkage, "TSL_TEXTURE2D_SAMPLE", context.module);
